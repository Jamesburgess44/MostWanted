  ...
  for(let i=0; i<searchCriteria.length; i++){
    ...
    switch(searchTrait){
      ...
      case "height":
        ...
        break;
      case "weight":
        let potentialMatchesWeight = searchByWeight(parseInt(searchValue), searchPool);
...
function searchByWeight(input, people){
  let potentialMatches = people.filter(function(potentialMatch){
    if(potentialMatch.weight == parseInt(input)){
      return true;
    }
    else{
      return false;
    ...
    for(let i=0; i<input.length; i++){
      if(input[i].includes('eyecolor ') || input[i].includes('height ') || input[i].includes('weight ') || input[i].includes('gender ')){
         if(input[i].includes('eyecolor ')){
           ...
           let height = parseInt(input[i].match(/(\d+)/));
          ...
          let weight = parseInt(input[i].match(/(\d+)/));
